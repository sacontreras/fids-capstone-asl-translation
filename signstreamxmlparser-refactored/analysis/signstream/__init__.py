...
class SignStreamDatabase(object):
  ...
  def _get_field_order(self):
    if self.f_order is None:
      self.f_order = list(self.fields.keys())
      self.f_order.sort()
    return self.f_order
  ...
  def _get_participant_order(self):
    if self.p_order is None:
      self.p_order = list(self.participants.keys())
      self.p_order.sort()
    return self.p_order
  ...
  def _get_media_order(self):
    if self.m_order is None:
      self.m_order = list(self.media.keys())
      self.m_order.sort()
    return self.m_order
...
def _strip_attrs(attrs):
  return dict((k, v.strip()) for (k, v) in list(attrs.items()))
