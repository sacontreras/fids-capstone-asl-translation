...
def load_video_index_dataset(debug=False):
  ...
  if not os.path.isfile(df_video_index_csv_path):
    ...
    print(f"{'SUCCESSFULLY saved' if fileio.path_exists(df_video_index_csv_path)[0] else 'FAILED to save'} {df_video_index_csv_path}")
...
def boostrap_signstream_corpus(d_corpus_info, df_video_index):
  ...
  if not os.path.isfile(d_corpus_info['corpus_ds_path']) \
    ...
    print(f"{'SUCCESSFULLY saved' if fileio.path_exists(d_corpus_info['corpus_ds_path'])[0] else 'FAILED to save'} {d_corpus_info['corpus_ds_path']}")
    df_asl_consultant.to_csv(path_or_buf=d_corpus_info['asl_consultant_ds_path'])
    print(f"{'SUCCESSFULLY saved' if fileio.path_exists(d_corpus_info['asl_consultant_ds_path'])[0] else 'FAILED to save'} {d_corpus_info['asl_consultant_ds_path']}")
    df_document_asl_consultant.to_csv(path_or_buf=d_corpus_info['document_asl_consultant_ds_path'])
    print(f"{'SUCCESSFULLY saved' if fileio.path_exists(d_corpus_info['document_asl_consultant_ds_path'])[0] else 'FAILED to save'} {d_corpus_info['document_asl_consultant_ds_path']}")
    df_video.to_csv(path_or_buf=d_corpus_info['video_ds_path'])
    print(f"{'SUCCESSFULLY saved' if fileio.path_exists(d_corpus_info['video_ds_path'])[0] else 'FAILED to save'} {d_corpus_info['video_ds_path']}")
    df_utterance.to_csv(path_or_buf=d_corpus_info['utterance_ds_path'])
    print(f"{'SUCCESSFULLY saved' if fileio.path_exists(d_corpus_info['utterance_ds_path'])[0] else 'FAILED to save'} {d_corpus_info['utterance_ds_path']}")
    df_utterance_video.to_csv(path_or_buf=d_corpus_info['utterance_video_ds_path'])
    print(f"{'SUCCESSFULLY saved' if fileio.path_exists(d_corpus_info['utterance_video_ds_path'])[0] else 'FAILED to save'} {d_corpus_info['utterance_video_ds_path']}")
    df_vocabulary.to_csv(path_or_buf=d_corpus_info['vocabulary_ds_path'])
    print(f"{'SUCCESSFULLY saved' if fileio.path_exists(d_corpus_info['vocabulary_ds_path'])[0] else 'FAILED to save'} {d_corpus_info['vocabulary_ds_path']}")
    df_utterance_token.to_csv(path_or_buf=d_corpus_info['utterance_token_ds_path'])
    print(f"{'SUCCESSFULLY saved' if fileio.path_exists(d_corpus_info['utterance_token_ds_path'])[0] else 'FAILED to save'} {d_corpus_info['utterance_token_ds_path']}")
...
def download_video_segment(segment_url, data_dir):
  # log_results = []
  if not fileio.path_exists(data_dir)[0]:
...
def extract_frames(segment_urls, video_fname, frames_dir, videos_dir, df_decomposition):
  ...
  if not fileio.path_exists(target_stitched_vid_frames_dir)[0]:
  ...
  if n_frames_expected > 0:
    ...
    n_stitched_frames = len(beam__common.list_dir(target_stitched_vid_frames_dir))
...
def run():
  ...
  print(f"{'SUCCESSFULLY saved' if fileio.path_exists(df_decomposition_csv_path)[0] else 'FAILED to save'} {df_decomposition_csv_path}")
