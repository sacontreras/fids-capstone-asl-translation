...
class Participant(object):
  ...
  def _get_utterance_order(self):
    if self.uorder is None:
      self.uorder = list(self.utterances.keys())
      self.uorder.sort()
    return self.uorder
  
  def __unicode__(self):
    return "%s: %s, %s user, age %s, id %d" % (self.name, self.gender, self.language,
                                                self.age, self.pid)
...
class Field(object):
  ...
  def __unicode__(self):
    return "Field %s: id %d" % (self.name, self.fid)
  ...
  def _get_value_order(self):
    if self.vorder is None:
      self.vorder = list(self.values.keys())
      self.vorder.sort()
    return self.vorder
...
class FieldValue(object):
  ...
  def __unicode__(self):
    return "Value %s: id %d, belonging to %s" % (self.name, self.vid, str(self.field))
...
class MediaFile(object):
  ...
  def __init__(self, mid, path):
    ...
    (dir, self.filename) = os.path.split(path)
    
  def __unicode__(self):
    return "Video %s: id %d" % (self.filename, self.mid)
...
class Utterance(object):
  ...
  def __unicode__(self):
    return "Utterance id %d, with %s" % (self.uid, str(self.participant))

  def _add_token(self, field, start, end, vid, text):
    fid = field.get_id()
    if fid not in self.tokens:
      self.tokens[fid] = []
  ...
  def _get_token_field_order(self):
    if self.torder is None:
      self.torder = list(self.tokens.keys())
      self.torder.sort()
    return self.torder
...
class Token(object):
  ...
  def __unicode__(self):
    ...
    if self.standard_token is not None:
      standard = " (*FID=%d, VID=%d)" % (self.field.get_id(), self.standard_token.get_id())
    return "%d-%d %s%s in %s" % (self.start, self.end, self.text, standard,
                                  self.field.get_name())
...
def _overlaps(xxx_todo_changeme, xxx_todo_changeme1):
